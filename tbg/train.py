...
if args.type in ["cv-condition", "cfg"]:
    ...
    encoder_layers = [75, 100, 100, 10, args.cv_dimension]
else:
...
for epoch in pbar:
    ...
    for it, idx in enumerate(batch_iter):
        ...
        if args.type == "cv-condition":
            ...
            s_t = cv_condition
            timelag_xyz = data_ac_timelag_xyz[idx]
            s_t_tau = tbgcv(timelag_xyz)
            s_t_centered = s_t - s_t.mean(dim=0)
            s_t_tau_centered = s_t_tau - s_t_tau.mean(dim=0)
            ac_num = s_t_centered.reshape(1, -1) @ s_t_tau_centered.reshape(-1, 1)
            ac_den = s_t_centered.norm(2) * s_t_tau_centered.norm(2)
            loss_ac = - ac_num / ac_den
            ...
            if torch.isnan(loss_ac):
                print(f">> Loss AC is NaN")
                print(f">> s_t: {s_t}")
                print(f">> s_t_tau: {s_t_tau}")
                print(f">> s_t_centered: {s_t_centered}")
                print(f">> s_t_tau_centered: {s_t_tau_centered}")
                print(f">> ac_num: {ac_num}")
                print(f">> ac_den: {ac_den}")
                raise ValueError(f"Loss AC is NaN")
...
if args.type in ["cv-condition-xyz", "cv-condition-xyz-ac"]:
    tbgcv.postprocessing = PostProcess(stats, tbgcv(c5_state[:, ALANINE_HEAVY_ATOM_IDX].reshape(1, -1))[0]).to(tbgcv.device)
elif args.type in ["cv-condition"]:
    c5_state = coordinate2distance(c5_state)
    tbgcv.postprocessing = PostProcess(stats, tbgcv(c5_state)[0]).to(tbgcv.device)
cv_normalized = tbgcv.postprocessing(cv.to(tbgcv.device))
print(f">> Max CV: {cv_normalized.max()}, Min CV: {cv_normalized.min()}")
# Save jit model
# tbgcv.trainer = Trainer(logger=False, enable_checkpointing=False, enable_model_summary=False)
# if args.type == "cv-condition":
#     input_dim = 30
# elif args.type in ["cv-condition-xyz", "cv-condition-xyz-ac"]:
#     input_dim = 10 * 3
# elif args.type == "cv-condition-xyzhad":
#     input_dim = 10 * 3 + 10
# random_input = torch.rand(1, input_dim).to(tbgcv.device)
# traced_script_module = torch.jit.trace(tbgcv, random_input)
# traced_script_module.save(save_dir + f"/{args.date}-jit.pt")
# print(f">> Model saved at {save_dir}/{args.date}-jit.pt")
